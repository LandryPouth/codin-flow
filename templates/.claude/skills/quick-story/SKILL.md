...
- **Reads**: story.md + Context Scope only
...
- **Skips**: Execution Packet, Context Map, Validation Gates, Rollback Notes, `decisions.md`

## Pipeline

1. Read story.md and Context Scope.
2. Run targeted searches from Context Scope `Search first`.
3. Read only the listed known relevant files until the edit point is clear.
4. Implement the change.
5. Run tests if available.
6. Update `implementation-notes.md` only if the change is non-trivial.
